...
var require_cookies = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var urllib$4 = __require("node:url");
...
var require_fetch = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var urllib$3 = __require("node:url");
	var zlib = __require("node:zlib");
	var PassThrough$3 = __require("node:stream").PassThrough;
...
var require_shared = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var urllib$2 = __require("node:url");
	...
	module.exports.resolveHostname = (options, callback) => {
		...
		if (dnsCache.has(options.host)) {
			...
			if (!cached.expires || cached.expires >= Date.now()) return callback(null, formatDNSValue(cached.value, { cached: true }));
		}
		resolver(4, options.host, options, (err, addresses) => {
			if (err) {
				if (cached) return callback(null, formatDNSValue(cached.value, {
					cached: true,
					error: err
				}));
				return callback(err);
	...
	*/
	...
	module.exports.parseDataURI = (uri) => {
		...
		if (!commaPos) return uri;
		let data = input.substring(commaPos + 1);
		let metaStr = input.substring(5, commaPos);
		let encoding;
		...
		for (let entry of metaEntries) {
			...
			if (sep >= 0) {
				...
				params[key] = entry.substring(sep + 1);
			}
		}
		switch (encoding) {
			case "base64":
	...
	function _handleAddress(tokens) {
		...
		for (i = 0, len = tokens.length; i < len; i++) {
			...
			if (token.type === "operator") switch (token.value) {
				...
				else data[state].push(token.value);
			}
		}
		...
		if (isGroup) {
			...
			if (!data.address.length && data.text.length) {
				for (i = data.text.length - 1; i >= 0; i--) if (data.text[i].match(/^[^@\s]+@[^@\s]+$/)) {
				...
				if (!data.address.length) for (i = data.text.length - 1; i >= 0; i--) {
...
var require_last_newline = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var Transform$5 = __require("node:stream").Transform;
...
var require_le_windows = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var Transform$4 = __require("node:stream").Transform;
	/**
...
var require_le_unix = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var Transform$3 = __require("node:stream").Transform;
	/**
...
var require_mime_node = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var PassThrough$2 = __require("node:stream").PassThrough;
	...
	var MailComposer = class {
		...
		*/
		getAttachments(findRelated) {
			...
			let attachments = [].concat(this.mail.attachments || []).map((attachment, i) => {
				...
				data = {
					contentType,
					contentDisposition: attachment.contentDisposition || (isMessageNode || isImage && attachment.cid ? "inline" : "attachment"),
		...
		*/
		_processDataUrl(element) {
			let parsedDataUri;
			if ((element.path || element.href).match(/^data:/)) parsedDataUri = parseDataURI(element.path || element.href);
...
var require_message_parser = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var Transform$2 = __require("node:stream").Transform;
	/**
...
var require_relaxed_body = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var Transform$1 = __require("node:stream").Transform;
...
var require_dkim = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var PassThrough$1 = __require("node:stream").PassThrough;
...
var require_http_proxy_client = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	*/
	...
	var urllib$1 = __require("node:url");
	/**
	...
	var XOAuth2 = class extends Stream {
		...
		*/
		getToken(renew, callback) {
			if (!renew && this.accessToken && (!this.expires || this.expires > Date.now())) return callback(null, this.accessToken);
...
var Aliyun, AOL, Bluewin, DebugMail, DynectEmail, Ethereal, FastMail, GandiMail, Gmail, Godaddy, GodaddyAsia, GodaddyEurope, Hotmail, iCloud, Infomaniak, Loopia, Maildev, Mailgun, Mailjet, Mailosaur, Mailtrap, Mandrill, Naver, One, OpenMailBox, Outlook365, OhMySMTP, Postmark, Proton, QQ, QQex, SendCloud, SendGrid, SendinBlue, SendPulse, SES, Seznam, Sparkpost, Tipimail, Yahoo, Yandex, Zoho, services_default;
		...
		*/
		_createConnection() {
			...
			connection.once("error", (err) => {
				if (err.code !== "EMAXLIMIT") this.logger.error({
					err,
					tnx: "pool",
					cid: connection.id
	...
	*/
	var SESTransport = class extends EventEmitter {
		...
		*/
		...
		_checkRatedQueue() {
			if (this.connections >= this.maxConnections || !this._checkSendingRate()) return;
		...
		_checkSendingRate() {
			...
			for (let i = this.rateMessages.length - 1; i >= 0; i--) {
				if (this.rateMessages[i].ts >= now - this.rateInterval && (!oldest || this.rateMessages[i].ts < oldest)) oldest = this.rateMessages[i].ts;
				if (this.rateMessages[i].ts < now - this.rateInterval && !this.rateMessages[i].pending) this.rateMessages.splice(i, 1);
			}
			if (this.rateMessages.length < this.sendingRate) return true;
		...
		*/
		_send(mail, callback) {
			...
			setImmediate(() => getRawMessage((err, raw) => {
				...
				let getRegion = (cb) => {
					if (ses.config && typeof ses.config.region === "function") return ses.config.region().then((region) => cb(null, region)).catch((err) => cb(err));
					return cb(null, ses.config && ses.config.region || "us-east-1");
				};
				getRegion((err, region) => {
					if (err || !region) region = "us-east-1";
					let sendPromise;
					if (typeof ses.send === "function" && aws.SendRawEmailCommand) sendPromise = ses.send(new aws.SendRawEmailCommand(sesMessage));
					else sendPromise = ses.sendRawEmail(sesMessage).promise();
		...
		*/
		verify(callback) {
			...
			let aws = this.ses.aws || {};
			const sesMessage = {
				RawMessage: { Data: "From: invalid@invalid\r\nTo: invalid@invalid\r\n Subject: Invalid\r\n\r\nInvalid" },
			...
			const cb = (err) => {
				if (err && (err.code || err.Code) !== "InvalidParameterValue") return callback(err);
				return callback(null, true);
			};
			if (typeof ses.send === "function" && aws.SendRawEmailCommand) {
				sesMessage.RawMessage.Data = Buffer.from(sesMessage.RawMessage.Data);
				ses.send(new aws.SendRawEmailCommand(sesMessage), cb);
			} else ses.sendRawEmail(sesMessage, cb);
			return promise;
		}
	};
...
var require_nodemailer = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	module.exports.createTransport = function(transporter, defaults) {
		...
		if (typeof transporter === "object" && typeof transporter.send !== "function" || typeof transporter === "string" && /^(smtps?|direct):/i.test(transporter)) {
			...
			else if (options.SES) transporter = new SESTransport(options);
